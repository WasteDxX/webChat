...
class ConnectWindow(CTk):
    def __init__(self):
        super().__init__()
        self.geometry('400x300')
        self.host = CTkEntry(self,placeholder_text='host')
        self.host.pack(pady = 20)
        self.port = CTkEntry(self, placeholder_text='port')
        self.port.pack(pady = 10)
        self.btn = CTkButton(self,text='Приєднатися',command=self.connect)
        self.btn.pack()

    def connect(self):
        self.hostName = conWin.host.get()
        self.portName = conWin.port.get()
        self.destroy()


conWin = ConnectWindow()
conWin.mainloop()
win = MainWindow(conWin.hostName,conWin.portName)
win.mainloop()
